...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT user_id, email, name, password_hash, is_admin, is_demo FROM users WHERE email = $1', [email.toLowerCase()]);
        ...
        res.json({ message: 'Login successful', user: { user_id: user.user_id, email: user.email, name: user.name, is_admin: user.is_admin, is_demo: user.is_demo }, token });
    } catch (error) {
...
// Demo Login — logs in as demo user, creates if needed
app.post('/api/auth/demo-login', async (req, res) => {
    try {
        const DEMO_EMAIL = 'demo@enoughfi.com';
        let result = await pool.query('SELECT user_id, email, name, is_admin, is_demo FROM users WHERE email = $1', [DEMO_EMAIL]);

        if (result.rows.length === 0) {
            // Create demo user
            const passwordHash = await bcrypt.hash('demo12345', 12);
            result = await pool.query(
                'INSERT INTO users (email, password_hash, name, is_demo) VALUES ($1, $2, $3, true) RETURNING user_id, email, name, is_admin, is_demo',
...
app.get('/api/auth/me', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT user_id, email, name, phone, is_admin, is_demo, created_at, last_login FROM users WHERE user_id = $1',
            [req.user.userId]
        );
...
const requireAdmin = async (req, res, next) => {
    try {
        const result = await pool.query('SELECT is_admin FROM users WHERE user_id = $1', [req.user.userId]);
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT u.user_id, u.email, u.name, u.is_admin, u.is_demo, u.created_at, u.last_login,
                   (SELECT COUNT(*) FROM transactions WHERE user_id = u.user_id) as transaction_count,
                   (SELECT COUNT(*) FROM accounts WHERE user_id = u.user_id) as account_count
...
app.put('/api/admin/users/:id/toggle-admin', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(
            'UPDATE users SET is_admin = NOT is_admin WHERE user_id = $1 RETURNING user_id, is_admin',
            [req.params.id]
        );
        if (result.rows.length === 0) return res.status(404).json({ error: 'User not found' });
        res.json(result.rows[0]);
...
app.delete('/api/admin/users/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        await pool.query('DELETE FROM staged_transactions WHERE user_id = $1', [userId]);
